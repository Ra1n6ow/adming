...
const (
	// recommendedHomeDir 定义放置 system 服务配置的默认目录.
	recommendedHomeDir = ".system"

	// defaultConfigName 指定了 system 服务的默认配置文件名.
	defaultConfigName = "system.yaml"
)
...
func initConfig() {
	...
	// 读取环境变量的前缀为 system，如果是 system 将自动转变为大写。
	viper.SetEnvPrefix("system")
...
// initStore 读取 db 配置，创建 gorm.DB 实例，并初始化 system store 层.
